...
const BusChart: React.FC<BusChartProps> = ({ dataSource }) => {
  ...
  if (!transportStats || !transportStats.bus || transportStats.bus.every(val => val === 0)) {
    return (
      <div className="d-flex justify-content-center align-items-center" style={{ height: '270px' }}>
        <p className="text-muted">No bus data available</p>
      </div>
    );
  }

  return <div id="busChart" ref={chartRef}></div>;
};
