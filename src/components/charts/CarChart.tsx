...
const CarChart: React.FC<CarChartProps> = ({ dataSource }) => {
  ...
  if (!transportStats || !transportStats.car || transportStats.car.every(val => val === 0)) {
    return (
      <div className="d-flex justify-content-center align-items-center" style={{ height: '270px' }}>
        <p className="text-muted">No car data available</p>
      </div>
    );
  }

  return <div id="carChart" ref={chartRef}></div>;
};
