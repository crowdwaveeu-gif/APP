...
class PresenceService {
  ...
  Future<void> _updatePresenceStatus(bool online) async {
    ...
    // Don't try to write to Firestore if the user is no longer authenticated
    // (e.g. during sign-out). This avoids PERMISSION_DENIED errors.
    if (_auth.currentUser == null) {
      if (kDebugMode) {
        print('Skipping presence update - user no longer authenticated');
      }
      _isOnline = false;
      return;
    }
  ...
  Future<void> _cleanupPresence() async {
    try {
      // Stop heartbeat first to prevent further writes
      _stopHeartbeat();

      // Only attempt Firestore write if user is still authenticated
      if (_currentUserId != null && _auth.currentUser != null) {
        await _updatePresenceStatus(false);
      }
      ...
      // Clear state even if write fails
      _currentUserId = null;
      _isOnline = false;
      if (kDebugMode) {
