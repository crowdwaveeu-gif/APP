...
class _UpdatedHomeScreenState extends State<UpdatedHomeScreen>
  ...
  void initState() {
    ...
    if (currentUser != null) {
      ...
      // No user yet - keep loading to prevent banner flash during auth transition
      _isKycCheckLoading = true;
    }
  ...
  Future<void> _checkKycStatus() async {
    ...
    if (_isCheckingKyc) return;

    final currentUser = _authService.currentUser;

    if (currentUser == null) {
      if (kDebugMode) {
        print('⚠️ No user - keeping loading state to prevent banner flash');
      }
      // Don't reset _isKycCheckLoading to false here. If the user is null
      // right now but about to log in (auth state in transit), resetting to
      // false causes a brief banner flash. The banner already hides when
      // currentUser == null, so there's no downside to keeping loading true.
      if (mounted && _hasSubmittedKyc != false) {
        setState(() {
          _hasSubmittedKyc = false;
        });
      }
    ...
    try {
      ...
      if (cachedStatus != null) {
        // We have a valid cache, only update if value changed
        if (mounted &&
        ...
        } else if (_isKycCheckLoading && mounted) {
          setState(() {
            _isKycCheckLoading = false;
          });
        }
  ...
  Widget _buildKYCBanner() {
    ...
    if (kDebugMode) {
      print(
          '⚠️ KYC Banner shown: hasSubmitted=$_hasSubmittedKyc, isLoading=$_isKycCheckLoading');
    }
